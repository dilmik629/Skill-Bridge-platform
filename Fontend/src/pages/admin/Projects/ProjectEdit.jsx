...
const ProjectEdit = () => {
  ...
  // useForm's internal state is only seeded from `initialValues` on the
  // very first render. Since the project data arrives asynchronously
  // (after the form has already mounted with empty defaults), we must
  // explicitly push it into the form once it's loaded, otherwise the
  // fields stay blank forever.
  useEffect(() => {
    if (initVals) reset(initVals)
  }, [initVals])
